...
namespace SampWeb
{
    ...
    internal sealed class Host : MarshalByRefObject, IRegisteredObject
    {
        ...
        public void Configure(WebServer server, int port, string virtualPath, string physicalPath, bool requireAuthentication, bool disableDirectoryListing)
        {
            ...
            NormalizedVirtualPath = CultureInfo.InvariantCulture.TextInfo.ToLower(virtualPath.EndsWith("/", StringComparison.Ordinal) ? virtualPath : (virtualPath + "/"));
            PhysicalPath = physicalPath;
            PhysicalClientScriptPath = HttpRuntime.AspClientScriptPhysicalPath + Path.DirectorySeparatorChar;
            NormalizedClientScriptPath = CultureInfo.InvariantCulture.TextInfo.ToLower(HttpRuntime.AspClientScriptVirtualPath + "/");
        }
        ...
        public void ProcessRequest(Connection connection)
        {
            ...
            try
            {
                var request = new Request(this, connection);
                request.Process();
            }
            finally
            {
        ...
        public SecurityIdentifier GetProcessSid()
        {
            ...
            using (var windowsIdentity = new WindowsIdentity(_server.GetProcessToken()))
            {
